...
/**
 * topic交换机模型，需要一个topic交换机，两个队列和三个Routing key
 */
@Configuration
public class TopicExchangeConfig {
    ...
    /**
     * 3个 routing key 将交换机和相应队列连起来
     * #表示0个或若干个关键字，*表示一个关键字
     */

    @Bean
    public Binding bindingtopic1() {
        Binding binding = BindingBuilder.bind(topicQueue1()).to(topicExchange()).with("*.orange.*");//routing key
        return binding;
    }
